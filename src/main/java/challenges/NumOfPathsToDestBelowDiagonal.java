 ...
 */
public class NumOfPathsToDestBelowDiagonal {
    ...
    static int numOfPathsToDest(int n) {
        ...
        return numsOfPathsToDestHelper(n - 1, n - 1, memo);
    }

    public static int numsOfPathsToDestHelper(int i, int j, int[][] memo) {
        if (i < 0 || j < 0) {
            return 0;
        } else if (i < j) {
            memo[i][j] = 0;
            ...
            memo[i][j] = numsOfPathsToDestHelper(i - 1, j, memo) + numsOfPathsToDestHelper(i, j - 1, memo);
        }
        return memo[i][j];
    }
